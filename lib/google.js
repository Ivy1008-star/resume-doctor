...
async function verifyJwt(token) {
  const crypto = await (async () => {
    try { return require('crypto') } catch { try { return (await import(/* webpackIgnore: true */ 'node:crypto')).default || await import(/* webpackIgnore: true */ 'node:crypto') } catch { return (await eval('import("node:crypto")')).default || await eval('import("node:crypto")') } }
  })()
